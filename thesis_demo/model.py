...
class Compl(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.fc1 = nn.Linear(2 + 1, n_hidden)
        self.fc2 = nn.Linear(n_hidden, n_hidden)

        self.enc_new_x = nn.Linear(2, n_hidden)
    ...
    def communicate_sparse(self, nodes, one_node):
        nn = len(nodes)
        q = self.Q(one_node)
        kk = [self.K(node) for node in nodes]
        vv = torch.stack([self.V(node) for node in nodes]).transpose(0,1)
    ...
    def forward(self, input_xx, input_yy, output_xx):
        ...
        node_qry = F.relu(self.enc_new_x(output_xx.squeeze(1)))

        nodes_enc = input_enc

        # step2: serveral rounds of msg passing
        for i in range(5):
            node_qry = self.communicate_sparse(nodes_enc, node_qry)

        # agg, _ = torch.max(torch.stack(nodes_enc), dim=0)
        
        # take the last node information
        agg = node_qry
    ...
    def predict(self, xx, yy, xx_new):
        ...
        xx_pos = to_positional(xx, xx_new)
        xx_pos_new = to_positional(xx_new, xx_new)
